...
export default function CreateTripPage() {
  ...
  useEffect(() => {
    async function loadResources() {
      try {
        const vRes = await apiFetch("/api/vehicles/available");
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-8 backdrop-blur-sm shadow-xl">
        <h1 className="text-3xl font-extrabold text-white mb-2">Create New Trip</h1>
        <p className="text-zinc-400 mb-8">Plan a route, assign a vehicle, and dispatch your cargo.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="flex items-center text-sm font-medium text-zinc-300">
                <MapPin className="w-4 h-4 mr-2 text-blue-400" /> Source Location
              </label>
              <input 
                required 
                type="text" 
                placeholder="e.g. New York, NY"
                className="w-full rounded-lg border border-zinc-700 bg-zinc-800/50 px-4 py-3 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all outline-none"
                value={source} 
            ...
            <div className="space-y-2">
              <label className="flex items-center text-sm font-medium text-zinc-300">
                <MapPin className="w-4 h-4 mr-2 text-red-400" /> Destination
              </label>
              <input 
                required 
                type="text" 
            ...
            {/* Distance (Auto) */}
            <div className="space-y-2 md:col-span-2 bg-zinc-800/30 p-4 rounded-lg border border-zinc-800 flex items-center justify-between">
              <div>
                <label className="block text-sm font-medium text-zinc-400">Planned Distance</label>
                <div className="text-xl font-bold text-white mt-1">
                  {loadingDistance ? (
                    ...
                    <span className="text-zinc-600 text-base font-normal">Enter locations to calculate</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="flex items-center text-sm font-medium text-zinc-300">
                <Weight className="w-4 h-4 mr-2 text-emerald-400" /> Cargo Weight (kg)
              </label>
              <input 
                required 
                type="number" 
            ...
            <div className="space-y-2">
              <label className="flex items-center text-sm font-medium text-zinc-300">
                <Truck className="w-4 h-4 mr-2 text-amber-400" /> Assign Vehicle
              </label>
              <select 
                required 
                className="w-full rounded-lg border border-zinc-700 bg-zinc-800/50 px-4 py-3 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all outline-none appearance-none"
                value={vehicleId} 
                ...
                {availableVehicles.map(v => {
                  ...
                  return (
                    <option key={v.id} value={v.id} disabled={isOverweight}>
                      {v.name} ({v.registrationNum}) - Max: {v.maxLoadCapacity}kg {isOverweight ? '(Capacity Exceeded)' : ''}
                    </option>
                  );
                })}
              </select>
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="block text-sm font-medium text-zinc-300 mb-1">Assign Driver</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {availableDrivers.length === 0 ? (
                  <div className="col-span-full p-4 border border-zinc-800 rounded-lg text-zinc-500 text-center bg-zinc-900/50">
                  ...
                  >
                    ...
                    <div className="text-xs text-zinc-500 mt-1">License: {d.licenseNumber}</div>
                  </div>
                ))}
              </div>
            </div>
